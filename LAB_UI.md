...
- **Revisión de relojes** (`src/consent/termsReview.ts`) — **no es preflight**: el preflight dice si `activate` entra, esto dice si conviene firmar. Corre sobre los términos antes de que exista firma, y separa dos cosas distintas. *Peligro*: un reloj en cero, que le regala el deal a una parte (PLURISWAP.md §3.8, cuatro casos pineados en `test/ZeroClocks.t.sol`); deshabilita los tres botones de firma y el de relayer hasta que se tilde el reconocimiento, y el tilde se borra en cada edición de los términos. *Piso de producción*: un juicio, no protocolo — los paths del catálogo (3600/1800/7200) lo rompen a propósito para poder recorrerse en una sesión, así que un warning acá es lo esperado. Bajo `PAYMENT_PROOF` sólo se revisa `fiatDuration` y se dice que el resto está inerte.
