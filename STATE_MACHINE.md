...
**El fallo sí se puede recuperar.** `_close` corre una sola vez — la escritura de `status` hace que todo verbo posterior revierta `WrongStatus` — así que un consumidor que falló ahí quedaría perdido para siempre si el kernel no guardara la deuda. Guarda: `Deal.postPending` es un bitmask de las llamadas post-terminal que faltan, y `Deal.closeH` / `closeP` / `bondAction` conservan el outcome que las parametriza (no es derivable de `status`: `STALEMATE` solo mapea a tres pares distintos de close/bond). `retryPostTerminal(dealId)` es permissionless y vuelve a intentar solo los bits pendientes. Es idempotente porque un bit se limpia **únicamente** cuando su propia llamada tuvo éxito, así que un reintento no puede aplicar dos veces un delta de reputación ni disponer dos veces el mismo lock. Sin esto, un módulo transitoriamente inalcanzable —un proxy en upgrade, una implementación pausada— le cuesta al sujeto para siempre: `Reputation.inFlight` queda consumido por un deal ya cerrado, y `BondVault.lockOf` mantiene `available` reducido, así que un bond depositado y ganado no se puede retirar.

Dos límites deliberados. El bit de bond se **abandona** (se limpia sin ejecutar) si el vault derivó de su `packageId` firmado o dejó de responder: TRUST-03 lo hace fail-open permanente con el lock quedando en el vault, y dejar el bit puesto significaría que `postPending` nunca llega a cero. El bit de reputación **no** se abandona: una notificación no es un cobro, y soltarla en silencio escondería la fuga de capacidad del sujeto; si el módulo no vuelve, el bit queda y `retryPostTerminal` sigue siendo llamable como no-op. Solo se escribe storage cuando algo quedó pendiente, así que un terminal limpio o un deal Core-only no pagan el camino de retry.
