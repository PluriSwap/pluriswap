...
- **Stats base (listado).** `attest_base`: prueba off-chain verificable `{handleCommit, tier, count, penaltyBand, expiry, repRoot}`. El listado muestra attestations frescas, no historiales. **Lo público son agregados**: el band (0 limpio / 1..5 / 6..15 / 16+) informa sin identificar, donde el contador crudo sería una huella digital.
...
| `attest_base` | `handleCommit`, `tier`, `count`, `penaltyBand`, `expiry`, `repRoot` + `token`, `decimals` (as-built F4: `penaltyBand` es el agregado de §3.15.7 con cotas invertidas — `band_claimed >= band_real`; la hoja es por token y `UNIT = 250·10^decimals` — un `decimals` privado inflaría el score por 10⁴; mismo patrón que `prepare_admit`, y el consumer cross-chequea contra el ERC-20) | Off-chain |
