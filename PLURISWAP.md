...
| `IPaymentVerifier` | (no es paquete: el rail) | `verify(PaymentClaim, proof) → (ok, paymentNullifier)` | — (su address entra al id del ZK) |
...
- `IPaymentVerifier` es el rail: `verify(PaymentClaim{dealId, fiatCommit, notBefore}, proof) → (ok, paymentNullifier)`, fail-closed. Detrás viven el circuito, el layout de public inputs, la reducción mod p del `dealId` y las **claves de confianza** (DKIM, notary, firma del banco). Cómo rotan es decisión de cada adapter y no toca ni el módulo ni el kernel. El nullifier sale del id de transacción del propio rail, así que dos evidencias de un mismo pago nombran el mismo nullifier.
...
**Rotación de claves (2026-09-24).** Todo adapter de rail hereda `PinnedAnchors` (`src/packages/adapters/PinnedAnchors.sol`). Las reglas:

1. **Claves fijas, sin registro.** Las anclas de confianza —la clave DKIM, la del notary, la de firma del banco, identificadas por el hash que expone el circuito— se fijan al desplegar el adapter, y no hay función que las cambie. La address del adapter está dentro del `packageId` ZK, así que las claves son parte de lo que firmaron las tres partes. Se descartó el registro mutable: quien puede **agregar** una clave puede falsificar un proof para cada deal vivo del paquete (un drain con firma), y quien puede **sacarla** deja varado a cada Provider honesto a mitad de deal (un kill switch).
2. **Cada clave tiene ventana.** `[validFrom, validUntil]`, a lo sumo `MAX_WINDOW = 400 días`. El adapter le pasa al circuito `[max(activatedAt, validFrom), validUntil]` y el circuito prueba que el pago cae adentro: una clave sólo cuenta para lo que pudo haber firmado, y el momento del pago nunca se publica. El techo acota una clave filtrada sin gobernanza: aun un paquete que nadie mantiene deja de confiar en una clave a los trece meses.
3. **Rotar es publicar, no editar.** Adapter nuevo con la clave nueva, `PaymentProof` nuevo, `packageId` nuevo; cualquiera puede desplegarlo (PERM-03). Los deals ya firmados siguen con su paquete (EXT-10) y liquidan bajo él hasta su `sunset` (el último `validUntil`); no hay nada que migrar.
4. **Sucesor pre-publicado.** Un adapter puede fijar la clave vigente y la siguiente (hasta `MAX_ANCHORS = 8`), con ventanas solapadas. Así una rotación planificada cruza un deal vivo sin dejarlo varado.
5. **Compromiso de una clave.** No hay freno de emergencia —sería un kill switch sobre los Provider honestos—. Se deja de firmar deals nuevos con ese paquete (el frontend lo deslista; eso no es un gate, PERM-05), y los deals en vuelo quedan expuestos hasta que terminen. La ventana acota cuánto.
6. **Rotación no planificada a mitad de deal.** Un pago firmado por una clave que nadie fijó no se puede probar, y el deal termina en el timeout. Es el riesgo declarado de este perfil (*el Provider que pagó y no obtuvo proof no tiene claim*). Las mitigaciones son del cliente.

**Obligación del cliente** (como los relojes de §3.8): no dejar firmar un deal ZK cuyo `activatedAt + fiatDuration` supere `verifier.sunset()`; comparar `anchors()` contra las claves vivas del rail (en DKIM, el registro DNS del selector) y advertir si la vigente no está fijada; y avisarle al Provider que pruebe antes del `fiatDeadline`, porque después compite con el timeout.
...
| Equilibrio de disputa de Core (cerrado) | Cerrado el 2026-09-22 (Parte IV): abandonar una disputa la pierde (`ABANDONED`), y un deal con tribunal cuesta pelear |
