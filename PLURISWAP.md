...
stateDiagram-v2
    ...
    DISPUTED --> STALEMATE: timeout sin ARBITRATION — deadlock, el principal se quema
...
| CASE-CORE-14 | `DISPUTED` | Split dual-firmado | Siempre | Rechaza (`SplitAfterDispute`), sin consumir nonces: después de una disputa sólo hay acuerdos de todo o nada. Un split antes de disputar sigue siendo CASE-CORE-09 |
| CASE-CORE-15 | `DISPUTED` | Cualquiera ejecuta el timeout | En o después de `disputeDeadline` | Sin ARBITRATION: `STALEMATE` (principal quemado a `0xdEaD`, locks de ambos al sink, `Deadlock` +10 a ambos). Con ARBITRATION: `ABANDONED` (principal entero al Provider; bonds se devuelven; el que abrió +5) |
...
| OUT-13 Deadlock | `STALEMATE` | **0/0: el principal se quema** (`PrincipalBurned`, a `0xdEaD`, credit-first); sin completion fee; locks de ambos al sink; `Deadlock` (+10) a ambos | Core sin `ARBITRATION` (off si `PAYMENT_PROOF`) |
...
| Equilibrio de disputa de Core | Decidido el 2026-09-24 (Parte IV): sin tribunal, sin split tras disputar y el deadlock quema el principal (equilibrio débil, medido); con tribunal, abandonar la pelea la pierde. La evidencia y el rediseño pendiente del tribunal están en `SIMULACIONES.md` |
...
- **`SIMULACIONES.md`** — la evidencia de incentivos: simulaciones de ecosistema contra los contratos reales en Anvil (`script/sim/`: estrategias fijas, largo plazo evolutivo, tribunal imperfecto), la literatura de teoría de juegos que las explica y el rediseño de la disputa con tribunal que se desprende. No normativo; las decisiones están en la Parte IV.
