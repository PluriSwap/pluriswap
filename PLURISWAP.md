...
- *Tags:* los literales `"rep" | "bond" | "handle" | "leaf"` son las constantes de campo fijadas `1 | 2 | 3 | 4`, respectivamente.
- *Salts derivados:* el salt de la hoja de cuenta **no lo elige el cliente**: es `Poseidon(sk_id, "leaf", version)` y los circuitos lo **exigen** (`register_account`, `prepare_admit`, `claim`). Reconstruir tu hoja exige reproducir su salt; mientras fue un parámetro libre, el salt vivía únicamente en el estado local del cliente, así que una cuenta no se recuperaba desde su secreto sino desde una base de datos — que se pierde mucho más fácil que una frase semilla, y se lleva puestas la reputación, el tier y los bonds gateados por `claimed`. Derivarlo no cuesta privacidad: la hoja ya se esconde detrás de `S`, y quien pueda recomputar el salt ya tiene `sk_id`. Se exige en vez de convenir porque un usuario de un cliente no conforme se entera el día que necesita recuperar, que es el único día en que no tiene arreglo. **El salt del handle sigue siendo libre a propósito**: rotarlo *es* la funcionalidad (§3.15.7).
