...
Offchain, siempre. El protocolo no la ejecuta, pero **sí la firma**: `DealTerms.fiatCommit` es un commitment oculto al pago acordado —rail, moneda, monto, cuenta del Holder, salt (§3.13)—, así que las dos partes acuerdan el mismo pago y un payment proof tiene contra qué probarse. El kernel nunca lo lee. El protocolo autentica proofs (ZK), espera acuerdos (dual-sign), respeta relojes o recibe veredictos (Kleros). La finalidad del fiat — chargebacks, Pix MED, transferencias reversibles — es riesgo de las partes, declarado en la política de disputas: la pata cripto es final, la pata fiat no lo es nunca.
...
6. **El dinero se mueve con culpa probada, acuerdo probado o desacuerdo elegido.** Sin veredicto no hay slash: el bond nunca va a la contraparte sin un tribunal que lo diga. Dos casos sin veredicto tienen consecuencia igual, y ninguna premia a nadie. **Con tribunal**, abandonar una pelea que abriste es perderla: tenías la corte y no la usaste, y el principal va entero a la otra parte. **Sin tribunal**, las dos partes eligieron no tener quien decida, y un desacuerdo que ninguna cede es un *stalemate*: el principal se parte, los locks de ambos se queman al sink y los dos quedan marcados. Es un detrimento razonable para cada uno, que existe para que acordar dentro de la ventana sea siempre mejor que el reloj.
...
Incompatibles al resolver: ZK + ARBITRATION. Reputación sin Passport. Bonds sin Passport + Reputación. ZK con `fiatCommit == 0` (`FiatCommitRequired`): sin pata fiat declarada no hay contra qué verificar, y el deal sólo podría terminar en `CANCELLED`.
...
| `STALEMATE` | Terminal | 50/50 de protocolo, sin veredicto. Tres orígenes: arbitraje rehusado o arbitration timeout (el tribunal no decidió; bonds se devuelven), o **timeout de `DISPUTED` en un deal sin ARBITRATION** (nadie quiso tribunal y nadie cedió; bonds de ambos al sink) |
...
stateDiagram-v2
    ...
    DISPUTED --> STALEMATE: timeout sin ARBITRATION — nadie quiso tribunal, nadie cedió
    DISPUTED --> ABANDONED: timeout con ARBITRATION — tenía corte y no la usó
...
| `disputeDeadline` | `DISPUTED` + dispute duration | Cualquiera ejecuta el timeout: sin ARBITRATION, `STALEMATE` (deadlock, bonds al sink, ambos marcados); con ARBITRATION, el que abrió la disputa la pierde (`ABANDONED`). Abrir arbitraje desde `DISPUTED` sólo **estrictamente antes** |
...
| `disputeDuration` | Abrir `DISPUTED` resuelve en el mismo bloque: sin ARBITRATION, stalemate inmediato; con ARBITRATION, forfeit inmediato | Sin tribunal, ambos: no hay ventana para acordar y el desacuerdo cuesta la mitad del principal, los bonds y la marca. Con tribunal, el Holder: su única defensa le entrega el principal entero |
...
| CASE-CORE-15 | `DISPUTED` | Cualquiera ejecuta el timeout | En o después de `disputeDeadline` | Sin ARBITRATION: `STALEMATE` (50/50, locks de ambos al sink, `Deadlock` +10 a ambos). Con ARBITRATION: `ABANDONED` (principal entero al Provider; bonds se devuelven; el que abrió +5) |
...
| OUT-13 Deadlock | `STALEMATE` | 50/50; **sin** completion fee; locks de ambos al sink; `Deadlock` (+10) a ambos | Core sin `ARBITRATION` (off si `PAYMENT_PROOF`) |
...
**Bonds** (si el paquete está seleccionado): viven en el BondVault, no en el escrow. Unlock en todo terminal pacífico, en el `STALEMATE` de un tribunal (OUT-11/12) y en `ABANDONED`. Slash sólo en OUT-09/OUT-10: lock del perdedor a la address de firma del ganador (nunca el Controller). **Quema sólo el deadlock (OUT-13)**: los locks de ambos van al sink inmutable, nunca a la contraparte, para que nadie gane dejando correr el reloj. Detalle en §3.14.5.

Claim no autentica fiat. Fiat timeout no es culpa del Provider. `ABANDONED` sí asigna culpa, pero asumida, no probada: por eso mueve el principal (que la parte que no abandonó habría cobrado igual sin la pelea) y no el bond, que sólo va a la contraparte con veredicto (II.6). El deadlock no asigna culpa a ninguno: los trata igual, porque ninguno aceptó tener quien decida. En un deal ZK, fiat timeout es la única salida sin proof: esperado, no un fallback extra.
...
Lo que no hay, y por qué: **ningún registro, ningún steward, ningún verbo de la DAO, ningún tribunal.** Quien puede *agregar* claves a un registro compartido puede falsificar un proof para cada deal que dependa de él, y quien puede *sacarlas* deja varados a los Provider honestos; un steward sería confianza continua; la DAO es recipient, nunca actor (II.8), y convertirla en quien decide qué pagos cuentan le daría el perfil de operador que la separación legal existe para evitar. La única autoridad del esquema es una parte sobre su propio riesgo (II.14).

Tres matices, declarados:

- **El perfil ZK deja de ser puramente determinista.** Aprobar una clave equivale, en la práctica, a una liberación voluntaria del Holder: económicamente inofensiva (su plata, al destino firmado, sin redirección posible, II.5), pero un momento de firma que se puede *phishear* (*"el banco rotó la clave, aprobá esta"*). Es la misma superficie que `release` ya tiene fuera de ZK; la defensa es la obligación de cliente de abajo.
...
Oculto y no en claro: los términos son públicos para siempre, el `payee` es una cuenta bancaria, y una moneda visible parte el conjunto de anonimato de §3.15.8 por país. El salt es lo que lo hace hiding — monto y moneda tienen poca entropía. Es la excepción deliberada al principio de arriba (la wallet muestra un `bytes32`), y por eso la legibilidad pasa a ser **obligación del cliente**, igual que los relojes de §3.8: un cliente conforme muestra el preimage en claro, recomputa el commitment y no deja firmar si no coincide con el de los términos. El kernel no interpreta el campo; lo firma, lo snapshottea y lo expone en `terms(dealId)`. En un deal Core sin ZK puede ser cero, y aun así declararlo vale: es la prueba firmada de qué pago se acordó, que un tribunal puede leer si las partes revelan el preimage.
...
**Un proof, el pago firmado.** El circuito prueba que el pago abre el `fiatCommit` de los términos (§3.13): un pago real pero distinto del acordado —otro monto, otra cuenta, otra moneda— no verifica.
...
`DISPUTED` se usa cuando el deal **no** seleccionó ZK: el Controller congela un claim no autenticado. Desde `DISPUTED`, en paz: mutual cancel (todo al Holder), co-signed release (todo al Provider), split (bps firmados; no es un veredicto, es un acuerdo parcial). Timeout de `DISPUTED`: cualquiera, tras `disputeDeadline`, lo ejecuta, y el resultado depende de si las partes firmaron tribunal. **Sin ARBITRATION** es un deadlock (OUT-13): `STALEMATE`, principal 50/50, locks de ambos al sink y `+10` a ambos. Nadie quiso quien decida y nadie cedió; el protocolo no finge saber quién tenía razón y le cobra el desacuerdo a los dos. **Con ARBITRATION** es un abandono (OUT-14): `ABANDONED`, principal entero al Provider. El Controller tenía corte y no la usó. El split dual-firmado, el co-signed release o abrir corte son las salidas *antes* de ese reloj, y el deadlock existe para que siempre convenga tomarlas.
...
| Sin veredicto (tribunal rehúsa, arbitration timeout) | Unlock de ambos: el tribunal falló, no las partes |
| Deadlock (timeout de `DISPUTED` sin ARBITRATION) | **Burn de ambos locks al sink.** Ninguno aceptó tener quien decida y ninguno cedió: los dos pagan. Al sink y no a la contraparte, para que nadie gane estancando |
...
- **El bond sube TU techo, no el del deal.** Como el cap que ata es el más chico de los dos lados (§3.14.7), un T4 con bond frente a un T1 sin bond sigue haciendo un deal de 250. No compra contrapartes: habilita a estar a la altura de las que ya pueden.
- **Tener bond no se puede exhibir de antemano.** El vault público expone `available`/`locked`, pero el privado los rechaza (`HiddenBalances`: un agregado por sujeto linkearía todos sus deals), y la capa de divulgación no prueba capacidad de bond — un attestation muestra tier, y el listado muestra las dos columnas como *potencial*. Lo que demuestra que hay bond es la activación misma: si el `reserve` entró, el lock existe. Un frontend que muestre "tiene bond" antes de eso está afirmando algo que nadie probó.

Principio: el dinero se mueve con culpa probada, con negativa probada a resolver, o con un desacuerdo que las partes eligieron no someter a nadie. El score registra el resto. `BondAction.Burn` tiene un solo productor, el deadlock (OUT-13): es el precio que hace poco atractivo dejar correr el reloj en un deal sin tribunal, y va al sink para que nadie cobre por estancar. Por la misma razón el abandono (OUT-14) devuelve los bonds: si el slash por abandono fuera a la contraparte, convendría estancar toda negociación para cazar el bond ajeno. Un split puede slashear de más sólo si **ambas partes lo firman**: eso es acuerdo, no culpa de protocolo. En un deal ZK no hay tribunal: el bond sirvió para subir el cap y se devuelve.
...
**Crédito por contraparte (2026-09-23).** Un deal completado suma **una sola vez por contraparte**: el primero con cada cuenta cuenta, los siguientes con la misma no. Los castigos **no** se deduplican nunca — un stalemate con alguien con quien ya operaste cuesta exactamente lo que cuesta un stalemate. La asimetría es la misma de §3.15.7 leída desde el otro lado: lo bueno se gana una vez por persona que lo avaló operando con vos, lo malo es simplemente cierto.
...
| `prepare_admit` | `dealSubject`, `newLeaf`, `nullRep(v)`, `token`, `principal`, `lockCommit`, `repRoot`, `decimals`, `pairTag` (as-built 2026-09-23: el acuerdo de par de §3.14.7 — el módulo compara los dos lados en `prepare`) | Bundle (si hay rep) |
| `claim` | `dealId`, `dealSubject`, `newLeaf`, `nullRep(v)`, `kind`, `token`, `principal`, `repRoot`, `pairTag`, `epoch` (as-built 2026-09-23: los dos que el prover no elige — el tag que el módulo guardó cuando ambos lados prepararon, y la época que sale de `block.timestamp`; un circuito no tiene reloj) | Post-terminal |
...
mocks/                      TestToken, FeeOnTransferToken, RevertingReceiver, Mock1271, PaymentVerifierMock, ArbitrationMock, PassportDecoderMock, PassportMock, HumanityVerifierMock, AccountVerifierMock, PreparePassportVerifierMock, PrepareAdmitVerifierMock, ClaimVerifierMock, DepositVerifierMock, PrepareBondVerifierMock, ReabsorbVerifierMock, WithdrawVerifierMock, DeadPassportMock, GatingMock, RelayerMock
```
...
| Capa privada sin desplegar | `script/DeployPrivate.s.sol` existe y simula limpio contra el estado real de Arbitrum Sepolia (68,7M gas, 19 txs, la mayor 7,1M). Falta el broadcast (necesita la key financiada). El wiring del lab dejó de ser pendiente cuando la UI salió del repo (2026-09-23) |
...
| Equilibrio de disputa de Core | Decidido el 2026-09-24 (Parte IV, sobre el 2026-09-22): sin tribunal, el timeout de una disputa es un deadlock que cuesta a ambos (50/50, bonds al sink, +10 a cada uno); con tribunal, abandonar la pelea la pierde |
