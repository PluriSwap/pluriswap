...
6. **El dinero se mueve con culpa probada o acuerdo probado.** Sin veredicto no hay slash. El stalemate por inacción quema bonds: ambas partes tenían salida y ninguna la tomó. *(La segunda mitad está en disputa abierta: la única salida del Provider que paga más del 50% necesita la firma de su contraparte. Parte IV, 2026-09-22, decisión ABIERTA.)*
...
| `disputeDuration` | Abrir `DISPUTED` es un `STALEMATE` inmediato | La única defensa del Holder cuesta la mitad del principal en el acto |
...
Las cuatro están pineadas contra el kernel (`test/ZeroClocks.t.sol` y `Packages.t.sol`), no inferidas de esta spec. **La protección es obligación del cliente**, y es una superficie de protocolo aunque no viva en el bytecode: un cliente conforme revisa los relojes antes de pedir una firma, distingue *peligro* (un cero: nunca es intencional) de *piso de producción* (un juicio), y no deja firmar un peligro sin un reconocimiento explícito. La implementación de referencia es `lab/src/consent/termsReview.ts` — función pura sobre los términos, para que cualquier cliente la corra.
...
Abrir `DISPUTED` es gratis en Core: sin fee y sin bond. Es el freno defensivo del lado Holder contra un claim no autenticado. No es tribunal, no es un win, no quema principal. **Tampoco es simétrico**: mata el `claim`, que es la única salida unilateral del Provider, y lo deja con el stalemate 50/50 como techo. Eso es una opción sin costo sobre la mitad del principal ajeno, y es la decisión ABIERTA de la Parte IV (2026-09-22); la conducta está pineada en `test/DisputeIncentives.t.sol`. El paquete de reputación oficial cobra un contest fee no-cero, una vez, de la wallet del opener (§3.14.6). Un clon con fee cero es otro hash. Court-only sin reputación sigue gratis.
...
| `attest_base` | `handleCommit`, `tier`, `count`, `expiry`, `repRoot` + `token`, `decimals` (as-built F4: la hoja es por token y `UNIT = 250·10^decimals` — un `decimals` privado inflaría el score por 10⁴; mismo patrón que `prepare_admit`, y el consumer cross-chequea contra el ERC-20) | Off-chain |
| `reveal_advanced` | `handleCommit`, `fields_mask`, `out_volume`, `out_penalty`, `requester`, `token`, `repRoot` (as-built F4: bit0 volume / bit1 penalty, `out = mask·campo` — selector aritmético; `requester` = pubkey hash del requester, 0 = pública bajo el handle) | Off-chain |
...
| 2026-09-22 | **ABIERTA** — el equilibrio de disputa de Core | Hallazgo de la revisión, verificado contra el kernel (`test/DisputeIncentives.t.sol`, `Packages.t.sol::test_providerCannotEscalateEvenWithArbitration`), **no** un bug: es el catálogo de §3.9 funcionando como está escrito. Un Provider que cumplió al 100% tiene como techo unilateral el **50%**. `openDisputed` es gratis en Core, lo puede abrir sólo el lado Holder, mata el `claim` —la única salida unilateral del Provider— y todo lo que paga más de la mitad desde `DISPUTED` necesita la firma del Controller. Seleccionar ARBITRATION no lo cambia: abrir corte también es Controller-only (§3.12.2), así que la misma parte tiene el freeze y la escalación. El principio II.6 justifica el 50/50 con *"ambas partes tenían salida"*, y para el Provider eso no se sostiene. Y el default no es neutral: en Core el Holder pone el 100% del principal y el Provider no pone nada on-chain, así que un 50/50 sobre un he-said-she-said mueve valor del que escrowea al que reclama — en las dos direcciones, que es lo que convierte a `openDisputed` en una opción y no en una defensa. Los costos existentes no se le acercan: contest 1% con piso ~2 USDC (§3.14.6), quema de bonds 10% por lado (§3.14.5). **Consecuencia sobre el espíritu:** hoy II.2 (*"Core no es un modo degradado: es el recinto"*) y la economía de Core se contradicen. Opciones sobre la mesa, ninguna gratis: (1) que el Provider también pueda abrir corte — bump de kernel, el catálogo de rulings sigue cerrado y el adapter sigue sin mover custodia; (2) bonds simétricos y grandes (≥50%) como condición del set oficial — mata el ataque y mata el acceso del T1; (3) stalemate no-50/50, función de quién tenía la carga — más fiel a "culpa probada", abre otros juegos; (4) declarar Core-only no apto para trade real y hacer ARBITRATION o ZK efectivamente obligatorias — coherente con el opt-in, rompe II.2 explícitamente. **No se cierra sin una entrada nueva.** Ninguna de las cuatro está implementada |
...
| ~~UI en deal ZK~~ | Cerrado: los predicados del lab apagan los cuatro verbos que el kernel guarda con `_requireNotZk` (`markFiat`, `claim`, `openDisputed`, `openCourt`), con un test de paridad contra esa lista |

### 5.7 Deployments

JSON bajo `deployments/`: `sepolia.json`, `sepolia-packages.json`, `sepolia-kleros*.json`, `sepolia-pool*.json`, `sepolia-ramp.json`, `sepolia-private.json`, `31337*.json`. Más de un escrow puede existir en Sepolia (core-only vs packaged; factory vieja vs nueva). Apuntar pools al escrow cuyo dominio firmaron. No mezclar ABIs. Los deploys de paquetes en testnet usan placeholders (`feeRecipient 0xFEE`, `sink 0xdeaD`, `TRIBUNAL 0x71B`): nunca son identidad de producción.
