...
export const VERB_DOCS: Record<string, VerbDoc> = {
  ...
  openCourt: {
    ...
    clock: "strictly-before: now < disputedAt + disputeDuration",
    money: "Nada del principal. msg.value = arbitrationCost exacto (Kleros) o approve del courtFee al mock.",
    why: "Escalar al jurado que el deal firmó. Siempre desde DISPUTED: no se salta la disputa.",
    packages: "Exige PKG_ARB. Incompatible con ZK. Desde FIAT_SENT: WrongStatus.",
  },
  readRuling: {
    verb: "readRuling",
    who: "Cualquiera",
    from: "ARBITRATION_ACTIVE",
    to: "RESOLVED_BY_ARBITRATION",
    money: "1 = Holder gana (refund, bond del Provider al Holder). 2 = Provider gana (payout, bond del Holder al Provider). 3 = no gana ninguno: 50/50 + completion fee, bonds unlock. Nunca STALEMATE.",
  ...
  forceArbitrationTimeout: {
    ...
    money: "No gana ninguno: 50/50 + completion fee. Bonds unlock, scores Silent.",
    why: "El tribunal nunca respondió. Mismo cierre que un refuse: no es STALEMATE.",
  },
  withdraw: {
    verb: "withdraw",
