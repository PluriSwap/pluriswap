...
**Estado: aspiracional.** Nada de esto está construido: la UI salió del repo el 2026-09-23 y todavía no
hay un rail elegido. Este archivo fija hacia dónde apuntar, no cómo es. Lo normativo —lo que un cliente
...
1. **Lenguaje de negocio, siempre.** *"Vas a recibir 1.500,00 BRL por Pix en la cuenta \*\*\*\*1234"*, no
...
3. **Las firmas viajan dentro de flujos que ya existen.** Aprobar la clave del banco no es un paso nuevo:
...
> **Vas a entregar 300 USDC y recibir 1.500,00 BRL**
> por Pix, en tu cuenta \*\*\*\*1234 (Banco X), de parte de *handle_del_provider*.
>
> Si el pago no se prueba antes de las 18:40 de hoy, los 300 USDC vuelven a tu wallet.
>
> [ Confirmar ]
...
2. Consulta la clave vigente con la que el banco firma sus comprobantes —en DKIM, el registro DNS del
...
5. Si no pudo verificar la clave: **no hay botón.** *"No pudimos confirmar la firma de mails de tu banco.
...
sola confirmación. Con una EOA son dos popups; el segundo tiene que explicarse solo: *"Autorizá a tu
banco actual a comprobar este pago"*.

### Si el banco rota su clave a mitad de deal

Notificación del propio deal, nunca un mensaje de la contraparte:

> **Tu banco actualizó su firma de mails.**
> La verificamos contra su servidor. Confirmá para que el pago de 1.500,00 BRL de este deal pueda
> acreditarse.
>
> [ Confirmar ]
...
> **Detectamos un cambio en la firma de tu banco que no pudimos verificar.**
> No firmes nada que te pidan por otro canal. Si el pago no se prueba antes de las 18:40, los 300 USDC
> vuelven a tu wallet.
...
| --- | --- | --- |
| **Verde** | *"Listo. Pagá 1.500,00 BRL por Pix a la cuenta \*\*\*\*1234. Tu comprobante va a poder probarse."* | Pagar |
| **Ámbar** | *"El banco del Holder cambió su firma. Esperando que el Holder la confirme."* | Esperar; el botón de pagar está bloqueado |
...
> **Recibiste el comprobante de tu banco.**
> [ Importar comprobante ] — o reenvialo a tu dirección de importación.
>
> Generando la prueba… (unos segundos; tu comprobante no sale de este dispositivo)
>
> **Listo.** Los 300 USDC están en tu wallet.
...
| --- | --- | --- |
| `FUNDED`, antes del deadline | *"Esperando el pago de 1.500,00 BRL"* | Semáforo + *"Pagá antes de las 18:40"* |
...
| `RELEASED` | *"Pago recibido. Entregaste 300 USDC"* | *"Cobraste 300 USDC"* |
| `CANCELLED` | *"Deal cancelado. Tus 300 USDC volvieron"* | *"Deal cancelado"* (+ si pagó y no probó: cómo reclamar el fiat por fuera del protocolo) |
...
- **Generar el proof en el dispositivo** (bb.js/WASM, LHF-8). Sin esto no hay *"tu comprobante no sale de
  este dispositivo"*.
- **Cómo entra el comprobante.** Importar un `.eml`, reenviarlo a una dirección de importación, OAuth con
  el proveedor de mail: cada uno cambia qué confianza y qué fricción hay.
