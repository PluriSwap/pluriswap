...
**Hecho:** ítems 1–3 y 5. Ítem 4: Passport y court oficiales listos (`HumanPassport`, `KlerosAdapter`); el verifier del path ZK sigue mock. Ítem 6 resuelto por otra vía: el edge de paquetes salió a la librería externa `Packages` (Escrow 22.2 KB → 16.4 KB). Decisiones de kernel del 2026-09-12 (§3.1, §3.5, `CLAIMED`, zero-checks) y contest-open del 2026-09-17 cerradas; ver §7.
...
| **Paquetes** | Opt-in. Trust *elegida*. Deben poder leer Core y validarse entre sí (mismo Passport, mismo vault, mismo operator). | El kernel los llama. `resolve` exige `reputation.passport() == mods.passport` y lo mismo en el vault (`PeerMismatch`). `admit` sigue re-identificando con *su* Passport. |
...
| Reputation → Passport / BondVault | Sí en `activate` | `PeerMismatch` si el Passport del módulo ≠ `mods.passport`. El vault llega como argumento de `admit`. |
| BondVault → Passport | Sí en `activate` | Mismo chequeo. Mezclar impls ya no activa. |
| ZK / Court → peers | ZK: address en el id | `PackageId.zk(module, verifier, feeRecipient, verifyFee)`. Court bindea adapter + partner + key. |
| Pool → Escrow | `IEscrow` | Lee `dealOf` / `settlementOf`. `authorize(ha, mods)` corre `Packages.resolve` y reserva `activationFee`. |
...
### 3.2 Fee de completion / ZK mayor que el leftover — **cerrado**

**Decisión:** `_invoice` omite el fee si `fee == 0 || fee >= left`. Un paquete degenerado no ladrilla el terminal. El kernel cobra los getters que entran al `packageId`, no `invoice*`.

### 3.3 Passport / Reputation / Bonds no son un solo sujeto — **cerrado en activate**

`Packages.resolve` exige el mismo Passport en Rep y Vault. Los `packageId` de reputación y bonds **siguen sin** incluir el Passport: el peer se chequea en vivo, no en el hash.

`admit` todavía re-identifica. `notifyTerminal` usa el sujeto snapshotado. Un remap post-activación no deja `inFlight` huérfano.

### 3.4 ZK: el `packageId` no bindeaba el módulo — **cerrado el id; el verifier sigue mock**

`PackageId.zk` ahora incluye la address del wrapper. Un clon que reporte el mismo V y los mismos fees es **otro** id.

`VerifierMock` sigue aceptando cualquier `abi.encode(dealId, nullifier)`. El camino ZK de testnet no es un proof. Pendiente de ítem 4.
...
### 3.6 Pool: tesorería incompleta y fees de paquete invisibles — **cerrado**

`credits` se escribe al reconocer el terminal. `authorize(ha, mods)` corre `Packages.resolve` y reserva `activationFee`. El approve al escrow es la suma exacta de las reservas, no `max`. `withdrawCredit` / `reconcile` son `nonReentrant`.
...
| Compose rampa → `activate` | `RAMPS.md` lo permite | Solo taxi (modelo / composer, no kernel) |
| `credits` del pool | Bucket de tesorería | **Hecho:** se incrementa al reconocer el terminal |
...
1. Swap del módulo ZK: **cerrado el id.** El verifier oficial sigue siendo mock: el path de testnet no es “solo V”.
2. `PassportMock` + `BondVault` en `DeployPackages` (Sepolia/local). En Arbitrum One el picker usa `HumanPassport`. Identity del mock no es trustless.
3. `invoice*` desacoplado del hash. **Cerrado:** el kernel cobra los getters del `packageId`.
4. Completion/ZK fee > leftover. **Cerrado:** se omite el fee (`fee >= left`).

**Medio**

5. `admit` re-identifica. Peers bindeados en `resolve`; `notify` ya no re-identifica.
6. `runPostTerminal` fail-open + `safeTransfer` en slash/burn. Si el ganador rechaza el ERC-20, el lock queda; `retryPostTerminal` no puede empujar a un receiver que revierte.
7. `Settlement.withdraw` transfiere y después pone el crédito en 0. Escrow y `Pool.withdrawCredit` son `nonReentrant`.
8. `ArbitrationMock` griefing / ruling público si se usa como court real. El path Kleros no tiene `submitRuling`.
9. Pool approve max + fees no reservados. **Cerrado.**
...
6. Recorte de storage del `Deal` cuando toque tamaño/gas. **Superado:** `Packages` (librería externa) saca resolve/engage/invoice/`runPostTerminal` del kernel. Tras contest-open el Escrow queda en 16.4 KB con 8.2 KB de margen. Los cuatro `uint8` de retry (`closeH`/`closeP`/`bondAction`/`postPending`) caben en el slot de `pkgs`.
...
8. Contest-open (2026-09-17). **Hecho.** Reputación oficial declara `contestBps=100` + `contestFloor=10e6` (1% con piso 10 USDC 6-dec) y lo bindea al `packageId`. El kernel cobra `contestDue` una vez al abrir `DISPUTED` o corte desde `FIAT_SENT`, de la wallet del opener. Core-only sigue gratis. Allowance corta: reject. Drift: fee 0. `STALEMATE` no invoicea completion.
